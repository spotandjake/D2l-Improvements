       ...
       case r:
        for (this.cancel = (async n => {
          const r = document.getElementById('main'), i = await fetch(`${n.organizationURL}${n.cid}`, {
            ...
            for (const t of l) {
              let r = 1 == t.ActivityType ? `${window.location.origin}${t.Url}` : t.Url;
              n.push({
          ...
          const h = async e => {
            ...
            'ChipFilterContent' == e.getAttribute('Category')) {
              ...
              let t = e.getAttribute('_url'), r = `<a href="${t}">Download</a>`;
              if (/\.(docx|jpg|mp4|pdf|png|gif|doc)$/.test(t) || /docs\.google\.com/.test(t)) if (/\.(jpg)/.test(t)) r = `<img width="100%" src="/d2l/api/le/${n.apiVersion.le}/${n.cid}/content/topics/${e.id}/file?stream=true">`; else if (/\.(mp4)/.test(t)) r = `\n            <video width="100%" height="auto" controls="">\n              <source src="${t}">\n              Your browser does not support the video tag.\n            </video>`; else {
          ...
          [ ...r.querySelector('section.StreamCards').children ].forEach((e => {
            ...
            'ChipFilterAssignments' == e.getAttribute('Category') && (e.querySelector('.FileFormAdd').addEventListener('click', (t => {
              const n = d.find((t => t.Id == e.id));
              console.log(n), u.show((t => {
                if ('picked' == t.action) {
            ...
            })), e.querySelector('.FileFormSubmit').addEventListener('click', (async t => {
              const n = await Promise.all([ ...e.querySelector('.UploadedFiles').children ].map((async e => {
                const t = e.getAttribute('documentid');
                return await u.export(t);
              }))), r = e.querySelector('.FileFormDescription').value;
              e.querySelector('.UploadedFiles').innerHTML = '', e.querySelector('.FileFormDescription').value = '';
              const i = {
                files: n,
                description: r
              };
              console.log(i), alert('Submitted');
            })));
          }));
  ...
  class q {
    ...
    search(e, {limit: t = -1} = {}) {
      ...
      return function(e, {ignoreFieldNorm: t = y.ignoreFieldNorm}) {
        ...
        const i = [];
        return n && i.push(j), r && i.push(H), e.map((e => {
          const {idx: n} = e, r = {
            item: t[n],
            refIndex: n
          };
    ...
    _searchLogical(e) {
      const t = R(e, this.options), n = (e, t, r) => {
        if (!e.children) {
