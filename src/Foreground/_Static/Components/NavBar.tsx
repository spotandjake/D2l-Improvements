...
const parseResponse = async (response) =>
  JSON.parse((await response.text()).trim().substr(9));
const NavBar = ({ brightSpace, showAside, setSearch }: props) => {
  ...
  const click = async (viewType: AlertShadeState) => {
    if (alertState === viewType || viewType == AlertShadeState.Closed) {
      ...
      const category = [
        '',
        AlertShadeState.Notification,
        AlertShadeState.Mail,
      ].indexOf(viewType);
      if (category != -1) {
        const response = await fetch(
            ...
            <div>
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: htmlDoc.querySelector('.vui-list').innerHTML,
                }}
              ></div>
            </div>
          </>
        ...
        setAlertContent(
          <ul className={styles.dropDown}>
            <li>
              <button
                onClick={() => {
                  localStorage.setItem('Extension-Disabled', 'true');
                  location.reload();
                }}
              >
                Disable Extension
            ...
            <li>
              <a href="https://durham.elearningontario.ca/d2l/logout">
                Log Out
              </a>
            </li>
          </ul>
    ...
    <nav className={styles.container}>
      ...
      <div className={`${styles.side} ${styles.leftSide}`}>
        <IconButton
          ...
          onClick={() => {
            if (showAside != undefined)
              setTimeout(() => showAside((prevState) => !prevState), 100);
          }}
        >
          <SvgIcon
      ...
      <div className={styles.center}>
        <input
          ...
          type="text"
          onChange={(e) => {
            if (setSearch != undefined) setSearch(() => e.target.value);
          }}
        />
      </div>
      <ClickAwayListener onClickAway={() => click(AlertShadeState.Closed)}>
        <div className={`${styles.side} ${styles.rightSide}`}>
          <IconButton
            ...
            className={[
              styles.IconButton,
              activeAlerts.includes('Mail') ? styles.Active : '',
            ].join(' ')}
            onClick={() => click(AlertShadeState.Mail)}
          >
            <SvgIcon
              className={styles.Icon}
              component={MailIcon}
          ...
          <IconButton
            ...
            className={[
              styles.IconButton,
              activeAlerts.includes('Message') ? styles.Active : '',
            ].join(' ')}
            onClick={() => click(AlertShadeState.Message)}
          >
            <SvgIcon
              className={styles.Icon}
          ...
          <IconButton
            ...
            className={[
              styles.IconButton,
              activeAlerts.includes('Alert') ? styles.Active : '',
            ].join(' ')}
            onClick={() => click(AlertShadeState.Notification)}
          >
            <SvgIcon
              className={styles.Icon}
          ...
          <IconButton
            aria-label="Profile"
            className={styles.IconButton}
            onClick={() => click(AlertShadeState.Profile)}
          >
          ...
          <div
            className={[
              styles.alertArea,
              alertState == AlertShadeState.Closed ? '' : styles.open,
            ].join(' ')}
          >
            {alertContent}
          </div>
